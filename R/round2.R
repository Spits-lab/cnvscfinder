...
OUT_DIR <- file.path(BASE, "cnv_results/VUB04/round7_processing")
...
metadata <- readRDS("/scratch/brussel/vo/000/bvo00016/vsc11567/cnv_framework/data/metadata0409_round7.rds")
chromosome_arms <- readRDS("/scratch/brussel/vo/000/bvo00016/vsc11567/cnv_framework/data/hg38_chromosome_arms.rds")
coding_genes <- readRDS("/scratch/brussel/vo/000/bvo00016/vsc11567/cnv_framework/data/hg38_expressing_genes.rds")


gene_level_df <- load_and_prepare_infercnv_reference(objs, k = 1.45)

metadata$cell_type <- "RPE"  
...
segments_chr1 <- segments %>%
  dplyr::filter(chr == "chr1")

cat("Chr1 segments:", nrow(segments_chr1), "\n")
...
cat("Chr1 after merge+density:", nrow(merged), "\n")
cat("Chr1 gain segments:\n")
print(merged %>%
  dplyr::filter(cnv_state == "gain") %>%
...
saveRDS(clustered_events, file = out_file)
